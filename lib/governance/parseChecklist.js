...
// table instead of a static list in the profile. Wins.category used to be
// checked this way against win_categories, but submissions span many
// different projects with no shared taxonomy — forcing every submitter into
// a fixed set (originally just AIRPAY's Platform/DCB/Digital Payment) meant
// a perfectly valid category like "Front End" got silently dropped instead
// of saved. Category is free text now; the hook stays in case another
// column needs a live-table enum later.
...
async function parseChecklistWorkbook(buffer, parserProfile) {
  ...
  for (const sheetName of SHEET_ORDER) {
    ...
    const { validRecords, unmappedRows: sheetUnmapped, warnings: sheetWarnings } = parseSheet(ws, def, null);
