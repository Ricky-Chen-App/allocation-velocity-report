...
// This org's own week-numbering runs one ISO week behind the calendar
// standard — the ISO week containing 2026-08-03 is 32, but the org calls
// that period "week 31". Every *displayed* week label goes through this
// shift before hitting isoWeek(); isoWeek() itself stays true ISO-8601.
// Shifting the date back 7 days (rather than doing "week number - 1")
// keeps year boundaries correct — a period that's ISO week 1 correctly
// rolls back into the previous year's last week instead of an invalid
// "week 0".
function companyWeekDate(date) {
...
function checklistFileName(projectKey, periodType, periodStartIso, schemaVersion) {
  ...
  const label = periodType === 'monthly'
    ...
    : `W${String(isoWeek(companyWeekDate(start))).padStart(2, '0')}`;
