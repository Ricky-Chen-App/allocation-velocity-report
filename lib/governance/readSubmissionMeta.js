...
function readMomMeta(buffer) {
  ...
  if (!m) return { error: 'missing_frontmatter' };
  let parsed;
  try {
  ...
  return { metaFields, sheetNames: null, sheetHeaders: null };
}

// Only checklist (.xlsx) and MoM-as-Markdown (.md) can be structurally
...
// never row content, per the file's top comment. Sheet/column checks only
// apply to checklist workbooks; MoM-as-Markdown is checked on Meta fields
// alone (its table structure isn't validated until Phase 6's parser exists).
function diffStructure(read, parserProfile, kind) {
  const differences = diffMetaFields(read.metaFields, parserProfile);
  if (kind === 'checklist') differences.push(...diffSheetColumns(read.sheetHeaders, parserProfile));
  return differences;
}
