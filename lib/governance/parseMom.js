...
async function parseMomWorkbook(buffer, parserProfile) {
  ...
  for (const sheetName of SHEET_ORDER) {
    ...
    // Wins.category is free text (see parseChecklist.js's parseSheet comment)
    // — no live-table enum check here.
    const { validRecords, unmappedRows: sectionUnmapped, warnings: sectionWarnings } = parseSection(sheetName, lines, def, null);
