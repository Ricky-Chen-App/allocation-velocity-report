...
function parseWeeklyUploadWorkbook(worksheet) {
  ...
  for (let idx = progressStartIdx; idx < rows.length; idx++) {
    ...
    if (texts.some(t => /^yesterday$/i.test(t || ''))) {
      ...
      // "Blocker / Notes" actually sits one row up, on the "Mon/Tue/.../Fri"
      // day-label row above this "Yesterday/Today/Move" row — check there
      // too, or the whole column (and every person's blocker note) is
      // silently dropped.
      if (map.blocker == null) {
        (rows[idx - 1] || []).forEach((t, i) => { if (/blocker/i.test(t || '')) map.blocker = i; });
      }
